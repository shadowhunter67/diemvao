...
import { auditInstitutionCatalog, institutionCoverage } from '../src/data/institutionCoverage.ts';
...
const catalogIssues = schoolFilter ? auditInstitutionCatalog().filter((issue) => issue.schoolId === schoolFilter || !issue.schoolId) : auditInstitutionCatalog();
...
if (catalogIssues.length > 0) {
  console.log('\nCatalog audit');
  for (const issue of catalogIssues.sort((a, b) => severityRank(a.severity) - severityRank(b.severity) || a.code.localeCompare(b.code))) {
    console.log(`  ${marker(issue.severity)} [${issue.severity.toUpperCase()}] ${issue.code}${issue.schoolId ? ` ${issue.schoolId}` : ''}`);
    if (verbose) console.log(`    ${issue.message}`);
  }
}

if (counts.errors > 0 || catalogCounts.errors > 0) process.exitCode = 1;
