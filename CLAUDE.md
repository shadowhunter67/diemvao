...
**Product identity**: UniscoreVN là nền tảng multi-school (kiến trúc). **HCMUT** có calculator thật (`status: 'supported'`). **UEH/UEL** có explorer + công cụ quy đổi ĐGNL (V-ACT) verified, đọc/ghi `ApplicantProfile` dùng chung, nhưng chưa có exact calculator cuối (`status: 'partial'`). **UIT** có trang thông tin + eligibility/bonus checker + điểm chuẩn thật, chưa có calculator chính xác (`status: 'researching'`). Các trường còn lại (HCMUS, USSH, IU, AGU, UHS) mới khai báo định danh trong registry. Mỗi trường có công thức/input schema/thang điểm hoàn toàn khác — KHÔNG ép mọi trường dùng chung một "universal formula engine". Xem README.md mục "Kiến trúc multi-school" + "HCMUT module".
